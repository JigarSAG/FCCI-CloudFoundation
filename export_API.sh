...
export_api() {
	...
	echo "Exporting API"
	cd $API_DIR
	apiDetails=$(curl --location $wmioapi_url/rest/apigateway/apis?apiName=$apiName \
				-H "x-HTTP-Method-Override: GET" \
				-H "Content-Type: application/json" -u $admin_user:$admin_password)

	apiId=$(echo "$apiDetails" | grep -o '"id":"[^"]*' | grep -o '[^"]*$')

	echo "Exporting the API with $apiName with apiId $apiId."
	echo $(pwd)
  	echo $(ls -ltr)

	curl -s $wmioapi_url/rest/apigateway/archive?apis=$apiId -d @"$API_DIR/export_payload.json" --output $API_DIR/apis/$apiName.zip -H "Content-Type: application/json" -H "x-HTTP-Method-Override:GET"  -u $admin_user:$admin_password
	unzip -o $API_DIR/apis/$apiName.zip -d $API_DIR/
	rm $API_DIR/apis/$apiName.zip
	cd $BIN_DIR
